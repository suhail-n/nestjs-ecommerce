...
export class ProductService {
    ...
    async findAll(): Promise<Product[]> {
        // the populate takes the reference id and populates it with the actual user
    ...
    async findById(id: string): Promise<Product> {
        const product: Product = await this.productModel.findById(id).populate('owner');
        if (!product) {
    ...
    async create(productDTO: CreateProductDTO, user: User): Promise<Product> {
        const product: Product = await this.productModel.create({ ...productDTO, owner: user });
        await product.save();
